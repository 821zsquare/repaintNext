...
from src.utils import IImage, poisson_blend
...
DEVICE = os.environ.get('HD_PAINTER_DEVICE', 'cuda:0')
...
GPU_QUEUE_ID = 'gpu_queue'
INPAINT_MODEL_ID = 'sd15_inp'
INPAINT_MODEL_LABEL = 'Stable-Inpainting 1.5'
...
has_example_assets = all(
    Path(example_1024).exists() and Path(example_2048).exists()
    for example_1024, example_2048, _ in example_inputs
)

# Load models for single-GPU local deployment.
sr_model = models.sd2_sr.load_model(device=DEVICE)
inp_model = models.load_inpainting_model(INPAINT_MODEL_ID, device=DEVICE, cache=False)
...
def upscale_run(
    ddim_steps, seed, session_id, img_index,
...
with gr.Blocks(css=demo_path / 'style.css') as demo:
    ...
    gr.Markdown(f"**Base model:** {INPAINT_MODEL_LABEL}")
    ...
    upscale_btn.click(
        ...
        outputs=[hires_image], 
        api_name="upscale",
        _js="function(a, b, c, d){ return [a, b, c, selected_gallery_index()] }",
        concurrency_limit=1,
        concurrency_id=GPU_QUEUE_ID
    )
